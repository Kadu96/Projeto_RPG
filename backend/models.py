...
class Feat(Base):
    ...
    feat_traits = Column(MutableDict.as_mutable(JSONB), nullable=False)
    is_active = Column(Boolean, nullable=False, default=True)
...
class AssCharactersTraits(Base):
    __tablename__ = "ass_characters_traits"

    ass_characters_traits_id = Column(Integer, primary_key=True, autoincrement=True, nullable=False, unique=True)
    character_id = Column(Integer, ForeignKey("characters.character_id", ondelete="CASCADE", onupdate="CASCADE"), nullable=False)
    trait_id = Column(Integer, ForeignKey("feats.feat_id", ondelete="CASCADE", onupdate="CASCADE"), nullable=False)
    ass_characters_traits_uuid = Column(String, nullable=False)
    is_enabled = Column(Boolean, nullable=False, default=False)
    is_active = Column(Boolean, nullable=False, default=True)

    # Índices conforme definido no DrawDB
    __table_args__ = (
        Index('ass_characters_traits_index_0', "trait_id", "is_active", "is_enabled", "character_id"),
        Index('ass_characters_traits_index_1', "character_id", "trait_id", unique=True),
    )